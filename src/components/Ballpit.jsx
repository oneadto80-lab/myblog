...
class x {
  ...
  #p() {
    ...
    this.canvas.getContext = (type, attributes) => {
      const context = originalGetContext(type, attributes);
      if (context && typeof context.getShaderPrecisionFormat === 'function' && !context.__ballpitPrecisionFallback) {
        ...
        context.getShaderPrecisionFormat = (...args) =>
          originalGetShaderPrecisionFormat(...args) ?? { rangeMin: 127, rangeMax: 127, precision: 23 };
        context.getParameter = parameter => {
          const value = originalGetParameter(parameter);
          if (value === null && parameter === context.VERSION) return 'WebGL 2.0';
          if (value === null && parameter === context.SCISSOR_BOX) return new Int32Array([0, 0, this.canvas.width, this.canvas.height]);
          if (value === null && parameter === context.VIEWPORT) return new Int32Array([0, 0, this.canvas.width, this.canvas.height]);
          return value;
        };
        context.__ballpitPrecisionFallback = true;
      }
      return context;
    };
...
class Z extends d {
  constructor(e, t = {}) {
    ...
    let n = null;
    try {
      const generator = new p(e, 0.04);
      n = generator.fromScene(s).texture;
      generator.dispose();
    } catch (error) {
      console.warn('Ballpit environment map unavailable; rendering without it.', error);
    }
    const o = new g();
    const r = new Y({ ...(n ? { envMap: n } : {}), ...i.materialParams });
    if (n) r.envMapRotation.x = -Math.PI / 2;
    super(o, r, i.count);
    this.config = i;
...
function createBallpit(e, t = {}) {
  const { rendererOptions, ...config } = t;
  const i = new x({
    canvas: e,
    size: 'parent',
...
const Ballpit = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    const init = (attempt = 0) => {
      ...
      if (!canvas || !parent || disposed) return;

      if ((parent.offsetWidth === 0 || parent.offsetHeight === 0) && attempt < 5) {
        frame = requestAnimationFrame(() => init(attempt + 1));
        return;
      }
      ...
      try {
        ...
        console.warn('Ballpit failed to initialize.', error);
      }
    };
